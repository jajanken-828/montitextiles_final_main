...
import {
    Users, Eye, CheckCircle, XCircle, UserPlus, Search, Filter,
    Calendar, Mail, Phone, Briefcase, MapPin, FileCheck, Upload,
    Trash2, ShieldCheck, Save, X, Plus, ChevronDown, ChevronUp
...
const addForm = useForm({
    ...
    position_applied: '',
    expected_salary: '',
    notice_period: 'immediate', // Lowercase to match DB
    textile_experience: '',
    status: 'pending',
    sss_file: null,
...
const addEmployment = () => employmentList.value.push({ company: '', years: '', salary: '', position: '', reason: '' });
...
const getStatusBadgeClass = (status) => {
    ...
    if (s === 'interview') return 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';
...
const acceptApplicant = () => {
    ...
    router.post(route('hrm.applications.accept', selectedApplicant.value.id), {
        module: selectedModule.value
    }, {
        preserveScroll: true,
        onSuccess: () => {
            triggerToast(`Applicant assigned to ${selectedModule.value} for interview.`);
...
const rejectApplicant = () => {
    ...
    router.post(route('hrm.applications.reject', selectedApplicant.value.id), {
        reason: rejectionReason.value
    }, {
        preserveScroll: true,
        onSuccess: () => {
            triggerToast(`Applicant ${selectedApplicant.value.name} rejected.`);
...
watch(() => addForm.first_name, (val) => sanitizeName(val, 'first_name'));
watch(() => addForm.middle_name, (val) => sanitizeName(val, 'middle_name'));
watch(() => addForm.last_name, (val) => sanitizeName(val, 'last_name'));
watch(() => addForm.city, (val) => sanitizeName(val, 'city'));
watch(() => addForm.state_province, (val) => sanitizeName(val, 'state_province'));

watch(() => addForm.street_address, (val) => {
    const filtered = sanitizeWithFallback(val, /[^a-zA-Z0-9\sñÑ.,#-]/g, 'street_address');
    if (val !== filtered) addForm.street_address = filtered;
});

watch(() => addForm.email, (val) => {
...
watch(() => addForm.postal_zip_code, (val) => {
...
// Submit Form
const addApplicant = () => {
    if (!/^[a-zA-Z\sñÑ-]+$/.test(addForm.first_name) || !/^[a-zA-Z\sñÑ-]+$/.test(addForm.last_name)) {
        triggerToast('First Name and Last Name must only contain letters.');
        return;
    }
    ...
    if (addForm.phone_raw.length !== 12 || !/^\d{12}$/.test(addForm.phone_raw)) {
        triggerToast('Phone number must be exactly 12 digits.');
        return;
    }

    if (!addForm.position_applied) {
        triggerToast('Please select the position applied for.');
        return;
    }

    if (!addForm.expected_salary) {
        triggerToast('Please enter the expected salary.');
        return;
    }

    if (!addForm.textile_experience) {
        triggerToast('Please specify if the applicant has textile experience.');
        return;
    }
    ...
    if (!street || !city || !state) {
        ...
        triggerToast(`Complete residential details are required. Missing: ${missing.join(', ')}`);
        return;
    }
    ...
    addForm.post(route('hrm.applications.store'), {
        ...
        onError: (errors) => {
            const msg = Object.values(errors)[0] || 'Failed to add applicant. Please check inputs.';
            triggerToast(msg);
        }
    });
};
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white uppercase tracking-tight">
                        Application <span class="text-blue-600">Management</span>
                    </h1>
                ...
                <button @click="openAddModal"
                    class="flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-sm transition-all shadow-lg shadow-blue-500/20 active:scale-95">
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                            <tr v-if="filteredApplicants.length === 0">
                                <td colspan="5" class="px-6 py-12 text-center text-sm text-slate-500">No pending
                            ...
                            <tr v-for="applicant in filteredApplicants" :key="applicant.id"
                                ...
                                <td class="px-6 py-4 text-sm font-semibold text-slate-600 dark:text-slate-300">{{
                                    applicant.position_applied }}</td>
                                <td class="px-6 py-4 text-sm text-slate-500">{{ formatDate(applicant.created_at) }}</td>
                                <td class="px-6 py-4">
                                    <span
                                        :class="[getStatusBadgeClass(applicant.status), 'px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider']">
            ...
            <div
                ...
                <div class="overflow-y-auto p-8 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h3 class="text-xs font-black text-blue-600 uppercase tracking-widest mb-3">Personal
                                Information</h3>
                            <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                                <div>
                                    ...
                                    <p class="text-sm font-bold">{{ selectedApplicant?.name }}</p>
                                </div>
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400">Email</p>
                                        <p class="text-xs">{{ selectedApplicant?.email }}</p>
                                    </div>
                                    <div>
                                        <p class="text-[10px] font-bold text-slate-400">Phone</p>
                                        <p class="text-xs">{{ selectedApplicant?.phone_number }}</p>
                                    </div>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Address</p>
                                    <p class="text-xs">{{ selectedApplicant?.street_address }}, {{
                                        selectedApplicant?.city }}, {{ selectedApplicant?.state_province }} {{
                                            selectedApplicant?.postal_zip_code }}</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            <h3 class="text-xs font-black text-emerald-600 uppercase tracking-widest mb-3">Application
                                Details</h3>
                            <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Position Applied</p>
                                    <p class="text-sm font-bold">{{ selectedApplicant?.position_applied }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Notice Period</p>
                                    <p class="text-sm uppercase font-bold text-amber-600">{{
                                        selectedApplicant?.notice_period?.replace('_', ' ') }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Expected Salary</p>
                                    <p class="text-sm">₱{{ selectedApplicant?.expected_salary }}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div v-if="selectedApplicant?.date_of_birth" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <h3 class="text-xs font-black text-blue-600 uppercase tracking-widest mb-3">Birth & Personal
                            </h3>
                            <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Date of Birth</p>
                                    <p class="text-sm">{{ formatDate(selectedApplicant.date_of_birth) }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Place of Birth</p>
                                    <p class="text-sm">{{ selectedApplicant.place_of_birth || 'N/A' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Civil Status</p>
                                    <p class="text-sm">{{ selectedApplicant.civil_status || 'N/A' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Religion</p>
                                    <p class="text-sm">{{ selectedApplicant.religion || 'N/A' }}</p>
                                </div>
                            </div>
                        </div>
                        <div>
                            <h3 class="text-xs font-black text-emerald-600 uppercase tracking-widest mb-3">Government
                                IDs</h3>
                            <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">SSS Number</p>
                                    <p class="text-sm">{{ selectedApplicant.sss_number || 'N/A' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">PhilHealth Number</p>
                                    <p class="text-sm">{{ selectedApplicant.philhealth_number || 'N/A' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Pag-IBIG Number</p>
                                    <p class="text-sm">{{ selectedApplicant.pagibig_number || 'N/A' }}</p>
                                </div>
                                <div class="grid grid-cols-3 gap-2 mt-2">
                                    <div v-if="selectedApplicant.sss_file_url"><a :href="selectedApplicant.sss_file_url"
                                            target="_blank" class="text-blue-600 text-xs">SSS ID</a></div>
                                    <div v-if="selectedApplicant.philhealth_file_url"><a
                                            :href="selectedApplicant.philhealth_file_url" target="_blank"
                                            class="text-blue-600 text-xs">PhilHealth ID</a></div>
                                    <div v-if="selectedApplicant.pagibig_file_url"><a
                                            :href="selectedApplicant.pagibig_file_url" target="_blank"
                                            class="text-blue-600 text-xs">Pag-IBIG ID</a></div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div v-if="selectedApplicant?.spouse_name || selectedApplicant?.mother_name"
                        class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div v-if="selectedApplicant?.spouse_name">
                            <h3 class="text-xs font-black text-purple-600 uppercase tracking-widest mb-3">Spouse
                                Information</h3>
                            <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Spouse Name</p>
                                    <p class="text-sm">{{ selectedApplicant.spouse_name }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Occupation</p>
                                    <p class="text-sm">{{ selectedApplicant.spouse_occupation || 'N/A' }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Address</p>
                                    <p class="text-sm">{{ selectedApplicant.spouse_address || 'N/A' }}</p>
                                </div>
                            </div>
                        </div>
                        <div v-if="selectedApplicant?.mother_name">
                            <h3 class="text-xs font-black text-purple-600 uppercase tracking-widest mb-3">Parents</h3>
                            <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Mother</p>
                                    <p class="text-sm">{{ selectedApplicant.mother_name }}<br />{{
                                        selectedApplicant.mother_address }}</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Father</p>
                                    <p class="text-sm">{{ selectedApplicant.father_name }}<br />{{
                                        selectedApplicant.father_address }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="selectedApplicant?.elementary_school">
                        <h3 class="text-xs font-black text-indigo-600 uppercase tracking-widest mb-3">Educational
                            Background</h3>
                        <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Elementary</p>
                                    <p class="text-sm">{{ selectedApplicant.elementary_school }} ({{
                                        selectedApplicant.elementary_year }})</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">High School</p>
                                    <p class="text-sm">{{ selectedApplicant.high_school }} ({{
                                        selectedApplicant.high_year }})</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">College</p>
                                    <p class="text-sm">{{ selectedApplicant.college }} ({{
                                        selectedApplicant.college_year }})</p>
                                </div>
                                <div>
                                    <p class="text-[10px] font-bold text-slate-400">Vocational</p>
                                    <p class="text-sm">{{ selectedApplicant.vocational }} ({{
                                        selectedApplicant.vocational_year }})</p>
                                </div>
                            </div>
                            <div>
                                <p class="text-[10px] font-bold text-slate-400">Special Skills</p>
                                <p class="text-sm">{{ selectedApplicant.special_skills || 'N/A' }}</p>
                            </div>
                        </div>
                    </div>

                    <div v-if="selectedApplicant?.employment_records && selectedApplicant.employment_records.length">
                        <h3 class="text-xs font-black text-amber-600 uppercase tracking-widest mb-3">Employment Records
                        </h3>
                        <div class="bg-slate-50 dark:bg-slate-900/40 p-6 rounded-2xl space-y-4">
                            <div v-for="(rec, idx) in selectedApplicant.employment_records" :key="idx"
                                class="border-b border-slate-200 dark:border-slate-700 pb-4 last:border-0">
                                <p class="text-sm font-bold">{{ rec.company }}</p>
                                <p class="text-xs">Years: {{ rec.years }} | Salary: {{ rec.salary }} | Position: {{
                                    rec.position }}</p>
                ...
                <div class="px-8 py-6 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                    <button @click="isViewModalOpen = false"
                        class="px-8 py-3 bg-slate-900 text-white rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-blue-600 transition-all">Close</button>
                </div>
            </div>
        </div>

        <div v-if="isAcceptModalOpen"
            class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md"
            @click.self="isAcceptModalOpen = false">
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl max-w-md w-full overflow-hidden">
                <div class="bg-emerald-600 p-8 text-white">
                    <h2 class="text-xl font-black uppercase">Accept Application</h2>
                    <p class="text-emerald-200 text-xs mt-1">Assign applicant to a department for interview</p>
                </div>
                <div class="p-8 space-y-6">
                    <div>
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Select
                            Module</label>
                    ...
                    <div class="flex gap-3">
                        <button @click="isAcceptModalOpen = false"
                            class="flex-1 py-4 text-slate-500 font-bold text-xs uppercase">Cancel</button>
                        <button @click="acceptApplicant" :disabled="!selectedModule"
                            class="flex-1 py-4 bg-emerald-600 text-white rounded-2xl text-xs font-black uppercase shadow-lg hover:bg-emerald-700 transition-all disabled:opacity-50">Confirm</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div v-if="isRejectModalOpen"
            ...
            <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] shadow-2xl max-w-md w-full overflow-hidden">
                <div class="bg-red-600 p-8 text-white">
                    <h2 class="text-xl font-black uppercase">Reject Application</h2>
                    <p class="text-red-200 text-xs mt-1">Provide reason for rejection</p>
                </div>
                <div class="p-8 space-y-6">
                    <div>
                        <label
                            class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 block">Reason</label>
                        <textarea v-model="rejectionReason" rows="3"
                            ...
                            placeholder="e.g., Insufficient experience..."></textarea>
                    </div>
                    <div class="flex gap-3">
                        <button @click="isRejectModalOpen = false"
                            class="flex-1 py-4 text-slate-500 font-bold text-xs uppercase">Cancel</button>
                        <button @click="rejectApplicant" :disabled="!rejectionReason.trim()"
        ...
        <div v-if="isAddModalOpen"
            ...
            <div
                ...
                <div
                    ...
                    <h2 class="text-xl font-black text-slate-900 dark:text-white uppercase tracking-tight">Add New
                        Applicant</h2>
                    ...
                    <form @submit.prevent="addApplicant" class="space-y-10">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div>
                                    ...
                                    <template v-else>
                                        <div class="flex flex-col items-center justify-center h-full">
                                            ...
                                            <p
                                                ...
                                                {{ addForm.image.name }}</p>
                                            <button @click="addForm.image = null" type="button"
                                ...
                                <InputError class="mt-1 text-red-500" :message="addForm.errors.image" />
                            </div>
                            ...
                            <div>
                                ...
                                <TextInput id="first_name" type="text"
                                    ...
                                    v-model="addForm.first_name" required placeholder="Juan"
                                ...
                                <InputError class="mt-1 text-red-500" :message="addForm.errors.first_name" />
                            </div>
                            <div>
                                ...
                                <TextInput id="middle_name" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.middle_name" placeholder="Santos"
                                    @keypress="blockNumbersAndSpecial($event, 'middle_name')" />
                                <p v-if="inputWarnings.middle_name"
                                    class="text-xs text-red-500 font-bold mt-1 ml-1 animate-pulse">{{
                                        inputWarnings.middle_name }}</p>
                                <InputError class="mt-1 text-red-500" :message="addForm.errors.middle_name" />
                            </div>
                            <div>
                                ...
                                <TextInput id="last_name" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.last_name" required placeholder="Dela Cruz"
                                    @keypress="blockNumbersAndSpecial($event, 'last_name')" />
                                <p v-if="inputWarnings.last_name"
                                    class="text-xs text-red-500 font-bold mt-1 ml-1 animate-pulse">{{
                                        inputWarnings.last_name }}</p>
                                <InputError class="mt-1 text-red-500" :message="addForm.errors.last_name" />
                            </div>

                            <div class="md:col-span-3 mt-4 border-t border-slate-200 dark:border-slate-700 pt-4">
                                <h3
                            ...
                            <div>
                                ...
                                <TextInput id="email" type="email"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.email" required placeholder="juan@example.com"
                            ...
                            <div>
                                ...
                                <div class="flex gap-2 mt-1">
                                    <select v-model="addForm.phone_country"
                                    ...
                                    <TextInput id="phone_raw" type="tel" maxlength="12"
                                        ...
                                        v-model="addForm.phone_raw" required placeholder="639123456789" />
                                </div>
                            ...
                            <div>
                                <InputLabel for="position_applied" value="Position Applied For"
                                    class="text-slate-700 dark:text-slate-300 font-semibold" />
                                <select id="position_applied" v-model="addForm.position_applied" required
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                            ...
                            <div>
                                ...
                                <TextInput id="expected_salary" type="number"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.expected_salary" required placeholder="e.g. 15000" />
                            </div>
                            <div>
                                <InputLabel for="textile_experience" value="Textile Experience"
                                    class="text-slate-700 dark:text-slate-300 font-semibold" />
                                <select id="textile_experience" v-model="addForm.textile_experience" required
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                            ...
                            <div>
                                <InputLabel for="notice_period" value="Notice Period"
                                    class="text-slate-700 dark:text-slate-300 font-semibold" />
                                <select id="notice_period" v-model="addForm.notice_period" required
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                            ...
                            <div class="md:col-span-3">
                                ...
                                <TextInput id="street_address" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.street_address" required placeholder="123 Main St, Brgy. San Jose"
                            ...
                            <div>
                                ...
                                <TextInput id="city" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.city" required placeholder="General Trias" />
                            </div>
                            <div>
                                ...
                                <TextInput id="state_province" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.state_province" required placeholder="Cavite" />
                            </div>
                            <div>
                                ...
                                <TextInput id="postal_zip_code" type="text" maxlength="4"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.postal_zip_code" required placeholder="4107" />
                            ...
                            <div>
                                ...
                                <TextInput id="place_of_birth" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.place_of_birth" placeholder="City, Province" />
                            </div>
                            <div>
                                ...
                                <TextInput id="citizenship" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.citizenship" placeholder="Filipino" />
                            </div>
                            ...
                            <div>
                                ...
                                <TextInput id="weight" type="number" step="0.1"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.weight" placeholder="65.5" />
                            </div>
                            <div>
                                ...
                                <TextInput id="height" type="number" step="0.1"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.height" placeholder="170" />
                            </div>
                            <div>
                                <InputLabel for="civil_status" value="Civil Status"
                                    class="text-slate-700 dark:text-slate-300 font-semibold" />
                                <select id="civil_status" v-model="addForm.civil_status"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                            ...
                            <div>
                                <InputLabel for="sex" value="Sex"
                                    class="text-slate-700 dark:text-slate-300 font-semibold" />
                                <select id="sex" v-model="addForm.sex"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                                    <option value="">Select</option>
                            ...
                            <div>
                                ...
                                <TextInput id="religion" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.religion" placeholder="Roman Catholic" />
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <TextInput id="sss_number" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.sss_number" placeholder="XX-XXXXXXX-X" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="philhealth_number" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.philhealth_number" placeholder="XX-XXXXXXXXX-X" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="pagibig_number" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.pagibig_number" placeholder="XXXX-XXXX-XXXX" />
                                </div>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div v-for="type in ['sss', 'philhealth', 'pagibig']" :key="type" class="space-y-2">
                                    <p class="text-[10px] font-black text-slate-500 uppercase ml-1">{{
                                        type.toUpperCase() }} ID Image</p>
                                    <div :class="addForm[type + '_file'] ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/10' : 'border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-900'"
                                        ...
                                        <template v-if="!addForm[type + '_file']">
                                            <Upload class="h-4 w-4 text-slate-400 group-hover:text-blue-500" />
                                            <input type="file" @change="(e) => handleFileUpload(e, type + '_file')"
                                        ...
                                        <template v-else>
                                            ...
                                            <p
                                                ...
                                                {{ addForm[type + '_file'].name }}</p>
                                            <button @click="removeFile(type + '_file')" type="button"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <TextInput id="spouse_name" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.spouse_name" placeholder="Juan Dela Cruz" />
                                </div>
                                <div>
                                    <InputLabel for="spouse_occupation" value="Occupation"
                                        class="text-slate-700 dark:text-slate-300 font-semibold" />
                                    <TextInput id="spouse_occupation" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.spouse_occupation" placeholder="Engineer" />
                                </div>
                                <div class="md:col-span-2">
                                    <InputLabel for="spouse_address" value="Address"
                                        class="text-slate-700 dark:text-slate-300 font-semibold" />
                                    <TextInput id="spouse_address" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.spouse_address" placeholder="Complete address" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 mt-6">
                            ...
                            <div v-for="(child, idx) in childrenList" :key="idx"
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <TextInput id="mother_name" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.mother_name" placeholder="Full name" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="mother_address" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.mother_address" placeholder="Address" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="father_name" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.father_name" placeholder="Full name" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="father_address" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.father_address" placeholder="Address" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <TextInput id="languages" type="text"
                                class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                v-model="addForm.languages" placeholder="Tagalog, English, etc." />
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <TextInput id="emergency_name" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.emergency_name" placeholder="Full name" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="emergency_relationship" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.emergency_relationship" placeholder="Spouse, Parent, etc." />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="emergency_phone" type="tel"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.emergency_phone" placeholder="09123456789" />
                                </div>
                                <div class="md:col-span-2">
                                    ...
                                    <TextInput id="emergency_address" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.emergency_address" placeholder="Complete address" />
                                </div>
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <TextInput id="elementary_school" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.elementary_school" placeholder="School name" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="elementary_year" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.elementary_year" placeholder="YYYY" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="high_school" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.high_school" placeholder="School name" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="high_year" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.high_year" placeholder="YYYY" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="college" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.college" placeholder="Course & School" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="college_year" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.college_year" placeholder="YYYY" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="vocational" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.vocational" placeholder="Course & School" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="vocational_year" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.vocational_year" placeholder="YYYY" />
                                </div>
                            </div>
                            <div>
                                ...
                                <TextInput id="special_skills" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.special_skills" placeholder="e.g., Microsoft Office, Sewing" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 mt-6">
                            <div class="flex items-center gap-2">
                                ...
                                <label for="has_employment_record"
                            ...
                            <div v-if="showEmploymentSection">
                                <div class="flex justify-between items-center mb-2">
                                    <h4 class="text-xs font-black text-blue-600 uppercase">Employment Records (from
                                ...
                                <div v-for="(rec, idx) in employmentList" :key="idx"
                        ...
                        <div class="grid grid-cols-1 gap-6 mt-6">
                            <div>
                                ...
                                <TextInput id="machine_operation" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.machine_operation" placeholder="e.g., Sewing machine, Forklift" />
                            </div>
                            <div>
                                ...
                                <TextInput id="referred_by" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.referred_by" placeholder="Name" />
                            </div>
                            <div>
                                ...
                                <TextInput id="referred_by_address" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.referred_by_address" placeholder="Address" />
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 mt-6">
                            ...
                            <div>
                                <InputLabel for="previous_employment_company"
                                    ...
                                    class="text-slate-700 dark:text-slate-300 font-semibold" />
                                <select id="previous_employment_company" v-model="addForm.previous_employment_company"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700">
                                    <option value="">Select</option>
                            ...
                            <div v-if="addForm.previous_employment_company === 'yes'"
                                ...
                                <div>
                                    ...
                                    <TextInput id="previous_employment_when" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.previous_employment_when" placeholder="e.g., 2020-2021" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="previous_employment_position" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.previous_employment_position" placeholder="Position held" />
                                </div>
                                <div>
                                    ...
                                    <TextInput id="previous_employment_department" type="text"
                                        class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                        v-model="addForm.previous_employment_department" placeholder="Department" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-6 mt-6">
                            ...
                            <div>
                                <p class="text-xs text-slate-500 mb-2">Are you at present or in the past related (by
                            ...
                            <div>
                                ...
                                <TextInput id="related_employees" type="text"
                                    class="mt-1 w-full py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900 border-none ring-1 ring-slate-200 dark:ring-slate-700"
                                    v-model="addForm.related_employees" placeholder="e.g., Juan Dela Cruz - Cousin" />
                            </div>
                        </div>

                        <div class="flex justify-end gap-3 pt-8 border-t border-slate-200 dark:border-slate-700 mt-8">
                            <button type="button" @click="isAddModalOpen = false"
                                class="px-6 py-3 text-slate-500 font-bold text-xs uppercase hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-all">Cancel</button>
                            <button type="submit" :disabled="addForm.processing"
                                ...
                                <span v-if="addForm.processing">Processing...</span>
